...
#define URI_SCHEME "dividendcash"
...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a DividendCash address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no DividendCash: URI
...
{
    // Convert dividendcash:// to dividendcash:
    //
    //    Cannot handle this later, because dividendcash:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open dividendcash.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "DividendCash.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for DividendCash.lnk
...
{
    return GetAutostartDir() / "dividendcash.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a dividendcash.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=DividendCash\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the dividendcash app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add dividendcash app to startup item list
